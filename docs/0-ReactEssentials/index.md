...
本文介绍 React 的新功能,例如服务器和客户端组件之间的差异等.

### 服务器组件(Server Components)

服务器和客户端组件允许开发人员构建跨服务器和客户端的应用程序，将客户端应用程序的丰富交互性与传统服务端渲染的优秀性能相结合.

### Thinking in 服务器组件

React 的服务器组件引入了一种新的思维模型，用于构建利用服务器和客户端的混合应用程序

现在你可以根据组件的用途灵活地选择在何处渲染组件，而不是在客户端渲染整个应用程序.

例如: 如果我们将页面拆分为更小的组件，您会注意到大多数组件都是非交互式的，并且可以作为服务器组件在服务器上呈现.对于那少部分的的交互式 UI，我们可以添加客户端组件.这与 Next.js 服务器优先的思想一致.

### Why 服务器组件?

所以，您可能会想，为什么用服务器组件？与客户端组件相比，使用它们有什么优势？

服务器组件使开发人员能够更好地利用服务器的基础架构.例如，您可以在服务器直接获取数据，并在服务器上保留客户端的 Js 依赖项，从而提高性能.服务器组件使编写 React 应用程序感觉类似于 PHP 或 RubyonRails，但具有 React 模板化 UI 的强大功能和灵活性.

使用服务器组件，初始页面加载速度更快，并且客户端 JavaScript 包大小减小.基本上客户端运行时的大小是可缓存且可预测的，并且不会随着应用程序的增长而增加.仅当通过在应用程序中使用客户端组件交互时，才会添加其他 JavaScript.
...
客户端组件使您能够向应用程序添加交互性.在 Next.js 中，它们在服务器上预渲染并在客户端上进行水合(hydrated,激活交互性).

#### "use client" 指令

“use client”指令用来声明客户端组件,以区分于服务器组件.
...
### Nesting 服务器组件 inside Client Components

考虑到上面概述的呈现流程，将服务器组件导入客户端组件是有限制的，因为这种方法需要额外的服务器往返.

#### Unsupported Pattern: Importing 服务器组件 into Client Components
...
#### Recommended Pattern: Passing 服务器组件 to Client Components as Props
...
// Pages in Next.js are 服务器组件 by default
...
export default function Page() {
  return (
    <div>
      ...
      {/* Error: `useState` can not be used within 服务器组件 */}
      <Carousel />
    </div>
  );
}
...
//  createContext is not supported in 服务器组件
...
### Rendering third-party context providers in 服务器组件
...
export default function RootLayout({ children }) {
  return (
    <html>
      <body>
        {/*  Error: `createContext` can't be used in 服务器组件 */}
        <ThemeProvider>{children}</ThemeProvider>
      </body>
    </html>
  );
...
### Sharing fetch requests between 服务器组件
